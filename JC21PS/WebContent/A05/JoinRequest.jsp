  	...
  	<table class='table table-bordered'>
  		...
  		<tr>
  			...
  			<button>タグのvalueオプションはボタンを押した際に登録処理に渡すリクエストパラメータの値を表す。
  			 --%>
  			<td colspan="1"><%= bean.getClubNameList() %></td>
  			<td colspan="1"><button type='submit' name='registClubId'  value='<%= bean.ClubId %>' onclick="return confirm('申請しますか？')">部員登録申請</button></td>
  		</tr>
  		<tr>
  			<td colspan ="2"><%= bean.getClubDescriptionList() %></td>
  		<%} %>
  		</tbody>
  	</table>
  	</div>
